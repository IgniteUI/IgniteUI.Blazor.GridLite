...
Ignite UI for Blazor Grid Lite accepts contributions, as long as they follow the guidelines explained below. When contributing you would have to follow these steps:
...
When working on an issue for the Ignite UI for Blazor Grid Lite repository, you need to be aware of and to follow a correct status workflow. We have created a number of status labels in order to communicate well what the current status of a single issue/pull request is. The statuses are as follows:

## Development - applicable to issues

### Statuses

1. `:new: status: new` this is the initial status of an issue. This label is placed automatically.
2. `:eyes: status: in-review` this is the status once someone picks up the issue and starts looking at it. Change the status to in-review when you pick it up.
3. `:hammer_and_wrench: status: in-development` this is the status once you start working on an issue. Assign the issue to yourself if it hasn't been assigned already and remove the previous status and assign it an in development status.
...
10. `:white_check_mark: status: resolved` this is the status of an issue that has been fixed and there are active pull requests related to it.

Example status workflows:

`:eyes: status: in-review` => `:hammer_and_wrench: status: in-development` => `:white_check_mark: status: resolved` (PR is created)

`:eyes: status: in-review` => `status: by-design` (Issue can be closed)

`:eyes: status: in-review` => `status: third-party-issue` (Issue can be closed)

`:eyes: status: in-review` => `status: not-to-fix` (Issue can be closed)
...
When logging issue you should assign `severity:` label. If you cannot determine the issue severity use the `:firecracker: severity: medium` as initial label and a developer will re-evaluate it when doing investigation.
...
2. `:firecracker: severity: medium` the issue is noticeable, but only in certain scenarios, which are not commonly anticipated. The issue affects end-user experience to a lesser degree, but does not affect the general functionalities of the feature. The feature behaviors may be affected by this issue when new behaviors are added, or new integration scenarios are developed.
3. `:fire: severity: high` the issue is noticeable in common interaction scenarios. The issue affects the end-user experience significantly, and affects one or more of the general functionalities of the feature. The feature behaviors are affected by the issue, causing one or more of the behaviors to malfunction.
4. `:boom: severity: critical` the issue appears in a core interaction scenarios (80% use-cases). The issue is immediately noticeable by end-users and stops their interaction flow (an exception is thrown which prevents the user to continue their work with the UI, or breaks the UI completely). The issues affects one or more of the core functionalities of the feature causing them or the entire feature to stop functioning.
...
Issues that need to be addressed immediately are marked with `:runner: priority: high` status.

## Testing - applicable to pull requests

1. `:x: status: awaiting-test` this is the initial status of pull requests. If you're performing the pull request, please place this status on it. Pull requests are accepted if and only if all status checks pass, review is performed, and the pull request has been tested and contains `:white_check_mark: status: verified`.
2. `:boom: status: in-test` place this status once you pick up the pull request for testing.
3. `:white_check_mark: status: verified` place this status once you've tested the pull request, have verified that the issue is fixed, and have included all necessary automated tests for the issue.
4. `:x: status: not-fixed` place this status once you've tested the pull request and you are still able to reproduce the issue it's attempting to fix. Then assign the developer back on the pull request.

Example status workflows:

`:x: status: awaiting-test` => `:boom: status: in-test` => `:white_check_mark: status: verified` (PR can be merged if all prerequisites are met)

`:x: status: awaiting-test` => `:boom: status: in-test` => `:x: status: not-fixed` => `:hammer_and_wrench: status: in-development` => `:x: status: awaiting-test`
...
4. `<issue>` - The issue is the reference of the github task you have. Be aware that you are able to link more than one issue. For instance `(#123 #456)`.
...
3. Run the formatting checks and tests and make sure they all pass.
...
2. Follow a test-driven development process (TDD) to ensure full code coverage.
   - Unit tests go in `tests/IgniteUI.Blazor.GridLite.Tests`: JS interop calls and their payloads in `GridLiteJsInteropTests`, rendered markup in `GridLiteRenderTests`, the wire format in `ModelSerializationTests` and `EnumSerializationTests`. Behavior in a real browser goes in `tests/IgniteUI.Blazor.GridLite.IntegrationTests` (NUnit + Playwright against the TestBed app).
3. Document all newly added public methods, inputs, outputs and properties.
4. Make sure all static code analysis and tests pass before opening a pull request. Note that trim-analysis diagnostics (IL2xxx) build as errors — see the ["Maintaining trim compatibility" section of docs/TRIMMING.md](../docs/TRIMMING.md#maintaining-trim-compatibility-contributors) for the policy on fixing versus suppressing them.
...
1. Describe in the appropriate section in the [CHANGELOG.md](../CHANGELOG.md)
...
1. Checkout the master branch locally. _Depending on the PR target this can also be a version branch._
