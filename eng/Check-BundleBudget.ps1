...
# performance evidence. Budgets are patterns and every file must match exactly one group -- an asset
# nobody budgeted for fails rather than passing silently.
[CmdletBinding()]
param(
...
if (-not (Test-Path -LiteralPath $Root -PathType Container)) {
    throw "Asset root '$Root' does not exist. Run 'npm run build' first."
}
