...
namespace IgniteUI.Blazor.GridLite.IntegrationTests.Infrastructure;
...
internal sealed class TrimmedPublishServer : IAsyncDisposable
{
    ...
    private static string EnsurePublished()
    {
        ...
        var wwwroot = Path.Combine(repoRoot, "tests", "IgniteUI.Blazor.GridLite.PublishSmoke",
            "bin", "Release", Tfm, "publish", "wwwroot");

        if (!File.Exists(Path.Combine(wwwroot, "index.html")))
        {
            var publish = new ProcessStartInfo("dotnet",
                $"publish tests/IgniteUI.Blazor.GridLite.PublishSmoke -c Release -f {Tfm} --nologo")
            {
